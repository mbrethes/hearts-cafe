...
/*! 
	...
	Module containing all routines to draw textured and solid and special case polygons
	and triangles. If you want kick ass double buffered full resolution 3D graphics on 
	your Gameboy Advance, use this module.
...
// ------------------------------------------------------------------------------------
// Public functions implemented in assembly. These are only documented here only 
// because doxygen can't handle documentation in .s files.
// ------------------------------------------------------------------------------------

/*!
	\brief Draws a solid filled 2D triangle.
	
	\param a_Triangle	Array of 3 \a SoVector2 objects, which are the three corner
						points of the triangle. All three need to be onscreen, in a fixed
						point format. This routine does not do any clipping.
	\param a_PaletteIndex	Color you want to draw the triangle with.

	This routine draws a filled (flat or solid or whatever you want to call it) triangle in
	the current \a SoMode4PolygonRasterizer backbuffer. This backbuffer is set by using
	the \a SoMode4PolygonRasterizerSetBuffer function.
*/
...
// ------------------------------------------------------------------------------------
// Private attributes;
// Sadly these need to be global cause we need them in the .c and 
// in the .s implementation. I could put them in another .h file, but that would result 
...
/*!
	...
	\warning This variable is only global because I need it in the .C and in the .S file.
			 Otherwise I would have declared it in the .C file instead. Sorry.
*/
