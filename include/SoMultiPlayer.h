...
/*! 
	...
	This module is for the GBA's multiplayer system where you can transfer packets from and to
	up to three other GBAs. It supports extensive error detection, and handles all possible
	failure cases. You can send a fair amount of bytes each frame, and it's very stable. If you
	lose a packet, it's because a cable got disconnected, or because you connected a third GBA
	in a 2 player session (for example).
...
/*!
	...
	big please, otherwise you won't be able to complete a single packet transfer 
	in a single frame. At a framerate of 60hz, I've had success with transfer 
	sizes up to 16 (32 bytes).
...
/*!
	...
	Ofcourse this is related to the packet size you are using. If you have a large packet size
	you might not be able to complete it at 60 hz. Just fiddle with this value, and 
	the \a SO_MULTI_PLAYER_PACKET_TRANSFER_SIZE value until you have a satisfactory result.
...
/*!
	\brief	Callback routine that is called whenever something is wrong with the communication 
	
	\return	Let it return \a TRUE if you want to continue trying, or \a FALSE if you want to stop
			trying.
*/
