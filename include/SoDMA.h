...
/*! 
	...
	The highest priority channel is DMA0, then DMA1, then DMA2, and finally DMA3.
	
	Suppose a DMA transfer is currently being executed, and another DMA transfer is 
	started with a higher priority. The current transfer will be halted, and it will resume
	upon completion of the higher priority DMA.
	
	There are several different uses for DMA, for which different channels are appropriate.

	- DMA 0 has the highest priority of the DMA channels. It can only be used from internal 
	memory (RAM) to any other internal memory (RAM). It can not be used to transfer from 
	ROM.

	- DMA channels 1 and 2 are for transfering between the ROM and RAM and the direct sound 
	FIFO. Direct-sound FIFO transfers can be accomplished only by using DMA 1 and 2. Therefore 
	whenever you use direct sound you will use DMA 1 or 2.

	- DMA 3 has the lowest priority and can be used to transfer data to and from any area 
	...
	- Set the amount of data you want to transfer (in 16 or 32 bit bursts depending on the transfer method.)
	- Specify the transfer method to be used.

	\warning	When \a SO_DMA_REPEAT is set, if V-blanking or H-blanking
				are selected as the moment of DMA startup, DMA is restarted when
				the next startup condition occurs (a VBlank or HBlank). In this mode,
				restarting will continue as long as the DMA enable flag is not set to 0
				using the \a SoDMAStop method

	\warning	When transferring data to OAM or OBJ VRAM by DMA during a H-blank,
				the HBlank must first be freed from OBJ display hardware processing
				periods using the \a SO_REG_DISP_CNT register.
