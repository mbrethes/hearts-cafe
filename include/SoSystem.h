...
/*! 
	...
	This module contains all important and most generic Socrates stuff. It is included by most
	other Socrates files.
...
#define SoSystemInitialize SO_DEPRECATED_FUNCTION(SoSystemInitialize, not_neccesary_to_call_anymore)
...
/*!
	...
	The SO_BIT_ * macros (where * is a number from 0 to 31) can be used to get a number
	in which specific bits are set. For example if you want a number in which bit 2 and
	bit 14 are set, you could use SO_BIT_2 | SO_BIT_14. This is the same as using 
	(1<<2)|(1<<14) but it is a lot more readable.

	Note that SO_BIT_0 is the least significant bit of a word, and SO_BIT_31 is the most
	significant bit.

	If you need more than one consequetive bits then check out the SO_ * _BITS macros too.
*///@{   
// ---------------------------------------
#define SO_BIT_0					(1)				
#define SO_BIT_1					(1<<1)			
...
/*!
	...
	The SO_ * _BITS macros (where * is a number from 2 to 32) can be used to get a number
	in which the first * bits are set. For example the binary number 000111 is equal 
	to SO_3_BITS. The binary number 0001111000 is equal to SO_4_BITS << 3.
