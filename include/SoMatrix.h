...
/*! 
	...
	To save memory we use a 3 row, 4 column matrix system. The missing 4th row is
	assumed to contain [0, 0, 0, 1]. This works because we only support
	homogeneous transforms (rotations, translations and uniform scales).

	All cells in a matrix are in fixed point format.

	Note that you rarely need to work with matrices. You should use the \a SoTransform
	structure instead, because it is better in dealing with accumulating precision problems.
	Even in the best case, if you multiply two orthogonal matrices a few hundred times,
	you can be sure they won't be orthogonal anymore.  This is even worse with a fixed-point
	number system as the errors crop up much faster.
