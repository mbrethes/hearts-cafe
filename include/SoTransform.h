...
/*! 
	...
	for accumulating errors, especially because we are using a low-precision fixed
	point format. That's why you should use this transformation class and not 
	convert it to a matrix until the very last moment.

	Note that the transform separately stores its translation, scaling, and rotation
	and they do not interact with each other.  When constructing a matrix from a
	transform, we always composite them in the order X rotation, Y rotation, Z rotation,
	translation, scale.  For this reason, transforms are not ideal for use in hierarchical
	scene graphs.
