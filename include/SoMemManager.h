...
/*! 
	...
	This file contains an implementation of a simple general purpose memory manager
	suitable for use in embedded applications.  It is based loosely on the GDC 2001
	talk "AGB Resource Management" by Rafael Baptista.  Credit goes to him for the
	basic algorithm used herein.
	...
	* The basic allocation block size must be a power of two.  Other sizes are
	  ...
	  minimum allowable block size is 8 bytes (in order to fit the free tree node).
	* The number of blocks in the free pool is the largest multiple of 8 that
	...
	preferably with code, or you find anything that is obviously wrong or grossly
	inefficient drop me a line.

	Configuration

    This implementation supports allocation block sizes equal to any power of two.
    The default (and minimum) block size is the size of the free node structure
    (8 bytes).  To allow varying block sizes, uncomment the \a SO_MEM_MANAGER_BLOCK_BITS
    #define in \a SoMemManager.h

    This implementation supports multiple active memory pools.  To force
    a single memory pool (and simplify the API slightly) uncomment the
...
//! \a SO_MEM_MANAGER_SINGLE_POOL_BASE is defined, then this
//! symbol must be defined as well.
