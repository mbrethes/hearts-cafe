...
/*! 
	...
	This module contains all single sprite functionality. It works together with 
	the \a SoSpriteManager and \a SoSpriteMemManager to create a complete sprite system.
	So before you start using \a SoSprite objects read about these other two modules too.

	One other thing. Sometimes sprites are referred to as objects or OBJ's. This is all the
	same.
...
/*!
	...
	You should never instantiate a SoSprite object yourself. It is useless. You should
	create a pointer to a SoSprite, and use the \a SoSpriteManagerRequestSprite function
	to make this pointer point to a valid sprite.
*/
typedef struct
{
	/*! 
		\brief		Attributes of the sprite. Actually
		
		\internal
		
		Actually you will only ever use the first 3 attributes. But because we 
		use these structures as overlays on OAM, we have to have a fourth attribute to fill 
		it up. This fourth attribute is actually part of one of the 32 rotation and scale 
		settings available. See the \a SoSpriteManager.c file for more information on this
		OAM (and shadow OAM, which is interesting).

		But you never work with these attribute directly anyway, you should use the \a SoSprite
		functions instead. So what do you care? :) hehe...
	*/
	u16  m_Attribute[ 4 ];	
